...
export const InfoOfGood = ({
    ...
    backHost
      ...
      let currentPrice = data.priceForOne[0][3];
      for (var i = 0; i < data.priceForOne.length;i++){
        if(parseInt(data.priceForOne[i][0])<=value && parseInt(data.priceForOne[i][2])>=value){
          if(i === 0){
            currentPrice = data.priceForOne[i][(data.priceForOne[i].length-1)];
          }
          else {
            currentPrice = data.priceForOne[i][(data.priceForOne[i].length-2)];
          }
        }
        else if (parseInt(data.priceForOne[i][0])<=value && i === (data.priceForOne.length-1)){
          currentPrice = data.priceForOne[i][1];
        };
      };
    let catValue = 0;
    if(count){
    ...
    return(
        ...
        <Grid item xs={12} sm={12} lg={12} >
                    ...
                    <div className={styles.analog}>{data.good.type}:</div>
                    {data.good.name}                     
                    </Box>
                  </Grid>
                  <Grid item lg={4.4} >
                    <Box sx={{Width: 270, textAlign:"center"}}>
                      <Box component="img" className={styles.imagemain}
                            src={data.good.avatarUrl ? `${backHost}${data.good.avatarUrl[image]}` : ''} />
                            <div className={styles.divimag}><Box component="img" className={styles.iamgeother}
                            src={data.good.avatarUrl ? `${backHost}${data.good.avatarUrl[0]}` : ''} alt="0" onMouseEnter ={(e)=>{setImage(parseInt(e.target.alt))}} onMouseLeave={(e)=>{setImage(parseInt(e.target.alt))}} /><Box component="img" className={styles.iamgeother}
                            src={data.good.avatarUrl ? `${backHost}${data.good.avatarUrl[1]}` : ''} alt="1" onMouseEnter ={(e)=>{setImage(parseInt(e.target.alt))}} onMouseLeave={(e)=>{setImage(parseInt(e.target.alt))}}/><Box component="img" className={styles.iamgeother}
                            src={data.good.avatarUrl ? `${backHost}${data.good.avatarUrl[2]}` : ''} alt="2" onMouseEnter ={(e)=>{setImage(parseInt(e.target.alt))}} onMouseLeave={(e)=>{setImage(parseInt(e.target.alt))}}/></div></Box> 
                  </Grid>
                  <Grid item lg={3.4} comtainer alignItems="stretch" direction="column"> 
                      ...
                      <Box sx={{display:"inline-block", m:"0 auto",minHeight:"28px"}}>
                        <ButtonGroup sx={{verticalAlign:"middle", margin:"0 auto"}}>
                          ...
                          <TextField inputProps={{style:{padding:0,height:"28px",textAlign:"center",width:"50px", fontSize:"12px",border:"0px"}}} defaultValue={value} value={value} onChange={e=>getText(e.target.value)}></TextField>
                      ...
                      <Table aria-label="customized table">
                        ...
                        <TableBody>
                        {data.priceForOne.map((row,key)=>(
                          <TableRow key={key} className={styles.rowsvoers}>
                            <TableCell 
                                      className={styles.pricecell}
                                      align="center"
                                      
                                      >{(data.priceForOne.length-1) === key ? 'от '+row[0] : row[0]+row[1]+row[2]}</TableCell>
                                      <TableCell 
                                      className={styles.pricecell}
                                      align="center"
                                      
                                      >{(data.priceForOne.length-1) === key ? row[1]+' руб.' : row[3]+' руб.'}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                      </Table>
                  ...
                  <div className={styles.maindiv}>
                    {selectBut === 'discription' && <div >
                      <button onClick={()=>setSelectBut("annotation")} className={styles.anotherannotation}>Показать аннотацию и оновную информацию по препарату "{data.good.name}"</button>
                      {data.good.about.map((obj,key)=>(
                        <div>
                  ...
                  {selectBut === 'annotation' &&  <div>
                      {data.good.annotation.map((obj,ind)=>(
                    ...
                    {selectBut === 'instruction' && <div >
                      <object  type="application/pdf" data={`${backHost}${data.good.description}`} style={{width:"100%",height:"800px"}}></object> 
                    </div>}
