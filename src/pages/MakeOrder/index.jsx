...
export const MakeOrderM = ({count,setCount,setCurrentPath}) =>{
    const [page,setPage] = useState(1);
    const [order,setOrder] = useState([]);
    const [delivery, setDelivery] = useState([0]);
    const [returnData, setReturnData] = useState();
    ...
    const MakeOrderButton = ((page)=>{
        if (page===5){
            return(
                <Link to='/' ><button className={styles.returninshopm} onClick={()=>setCurrentPath('/')}>Вернутся в магазин</button></Link>
            );
        }
        else{
            return(
    ...
    <div>
        ...
        <Box sx={{ flexGrow: 1 }} className={styles.root}>
            <Grid container spacing={1} direction="row">
            <Grid item xs={12} md={8}  style={{minHeight:"100%", margin:"0 auto"}}>
                <Link to={'/'} onClick={()=>setCurrentPath('/')}><img src={Image} alt='One Pill' style={{width:"90px",height:"90px", margin:"0 auto", verticalAlign:"middle",display:"inline-block"}} />
                    <div className={styles.name}>
                    <div className={styles.head}>One Pill
                    </div>
        ...
        <div className={styles.wayorderhead}>Оформление заказа</div>
            <div className={styles.wrapper}>
                <ol className={styles.cstepper}>
                    <li className={styles.cstepper__item}><h9 className={page===1?styles.numberactive:styles.numberdone}>1</h9></li> 
                    <li className={styles.cstepper__item} ><h9 className={page ===2 ? styles.numberactive : (page>2 ? styles.numberdone:styles.numbernonactive)}>2</h9></li>
                    <li className={styles.cstepper__item} ><h9 className={page ===3 ? styles.numberactive : (page>3 ? styles.numberdone:styles.numbernonactive)}>3</h9></li>
                    <li className={styles.cstepper__item} ><h9 className={page ===4 ? styles.numberactive : (page>4 ? styles.numberdone:styles.numbernonactive)}>4</h9></li>
                    <li className={styles.cstepper__item} ><h9 className={page ===5 ? styles.numberdone:styles.numbernonactive}>5</h9></li>
                </ol>
            </div>
            {renderSwitch(page)}
        ...
        <div className={styles.buttonall}> 
            <div className={styles.backbutton}>{page!==1&&page!==5 ? <button className={styles.back} onClick={backPage}>Назад</button>:''}</div>
             <div className={page===5 ? styles.nextbuttonm:styles.nextbutton}>{MakeOrderButton(page)}</div>
        </div>
        {page!==5?
        (
            <>
            <div className={styles.wayorderhead}>Информация о заказе</div>
        <div className={styles.detailmainorder}>
            <div className={styles.productlistm}>
            {count.map((obj,ind)=>(
                <>
            <div className={styles.mainorder}>
                <a href={`/good/${obj.id}`}><img src={`${backHost}${obj.avatar}`} className={styles.imagekda} alt='img'></img></a>
                ...
                <div className={styles.priceorder}>
                    <div className={styles.priceorder}>
                        <div className={styles.totalpriceorderm}>{obj.cnt} шт.</div>
                        <div className={styles.calculorderm}> х {obj.maxPrice}</div>
                    </div>
                </div>
            ...
            <div className={styles.totalsumcntm}> 
              <div className={styles.ts}>
              ...
              <div className={styles.ts}>
                ...
                <div className={styles.ts}>
                <div className={styles.descripttotal}>К оплате: </div> <div className={styles.totalpaym}>{totalSum} </div>
                </div>
              </div>
        </div></>
        )
